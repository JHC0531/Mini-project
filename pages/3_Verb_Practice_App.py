...
VERB_LIST_PATH = find_data_file("verb list.csv")
...
def load_question_data():
    if not DATA_PATH.exists():
        st.error(f"data.csv 파일을 찾을 수 없습니다. 현재 찾는 위치: {DATA_PATH}")
        st.stop()
    ...
    if missing:
        ...
        st.info(f"현재 data.csv를 찾은 위치: {DATA_PATH}")
        st.stop()

    # 선택 사항: data.csv에 image_file 열을 추가하면 문항별 이미지를 직접 지정할 수 있습니다.
    # image_file 열이 없어도 images/q001.png, images/1.png 같은 파일명을 자동으로 찾습니다.
    ...
    df["image_file"] = df["image_file"].apply(normalize_text)
...
def find_question_image(question_id, image_file=""):
    """문항 이미지 파일을 찾습니다.
    ...
    │   └── ...
    └── pages/
        └── 3_Verb_Practice_App.py

    data.csv에 image_file 열을 만들고 파일명을 직접 적어도 됩니다.
    예: images/q001.png 또는 q001.png
    """
    qid = int(question_id)
    ...
    if image_file:
        given = Path(str(image_file).strip())
        if given.is_absolute():
            ...
            candidates.extend([
                ROOT_DIR / given,
                APP_DIR / given,
                IMAGE_DIR / given.name,
            ])

    for ext in extensions:
        candidates.extend([
            IMAGE_DIR / f"q{qid:03d}{ext}",
            IMAGE_DIR / f"q{qid}{ext}",
            IMAGE_DIR / f"{qid}{ext}",
            IMAGE_DIR / f"id_{qid:03d}{ext}",
            IMAGE_DIR / f"id_{qid}{ext}",
        ])
...
question_image = find_question_image(qid, current.get("image_file", ""))
if question_image:
    st.image(str(question_image), use_container_width=True, caption="Picture clue")
else:
    with st.expander("이미지 파일이 아직 없어요"):
        st.write("이미지를 넣고 싶으면 저장소 가장 바깥에 images 폴더를 만들고, 문항 번호에 맞춰 파일을 넣어주세요.")
        st.code("images/q001.png\nimages/q002.png\nimages/q003.png", language="text")
        st.caption(f"현재 이 문항에서 찾은 이미지 폴더: {IMAGE_DIR}")
...
with st.expander("데이터 점검 결과"):
    ...
    st.caption(f"data.csv 위치: {DATA_PATH}")
    st.caption(f"verb list.csv 위치: {VERB_LIST_PATH}")
    st.caption(f"pattern guide.xlsx 위치: {PATTERN_GUIDE_PATH}")
    st.caption(f"images 폴더 위치: {IMAGE_DIR}")
