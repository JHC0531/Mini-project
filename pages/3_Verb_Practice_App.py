...
st.markdown("")
pattern_col1, pattern_col2 = st.columns([1, 2])
with pattern_col1:
    st.markdown(f"<div class='pattern-box'>{escape(str(pattern_label))}</div>", unsafe_allow_html=True)
with pattern_col2:
    if pattern_structure:
        st.markdown(
            f"<div class='pattern-box'>기본 구조: {escape(str(pattern_structure))}</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown("<div class='pattern-box'>기본 구조 확인</div>", unsafe_allow_html=True)
